...
async function listDoctors(pageInput, limitInput, searchInput, activeUsersOnly, scope) {
  const pool = getPool();
  ...
  const { clause: clinicClause, params: clinicParams } = sqlClinicColumn('d.clinic_id', scope);

  const conditions = [clinicClause];
...
async function createDoctor(payload, actorId, clinicId) {
  const pool = getPool();
  ...
  try {
    ...
    const [userInsert] = await connection.query(
      `INSERT INTO users (username, email, password, role_id, is_active, organization_id, clinic_id)
       VALUES (?, ?, ?, ?, 1, ?, ?)`,
      [payload.username, payload.email, generatedPassword, doctorRoleId, orgId, cid]
    );
    ...
    await connection.query(
      `INSERT INTO doctor (
        ...
        available_days, available_time, profile_image_id, clinic_id
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        userId,
...
async function updateDoctor(doctorId, payload, actorId, scope) {
  const pool = getPool();
  const connection = await pool.getConnection();
  ...
  try {
    ...
    const [doctorRows] = await connection.query(
      `SELECT id, user_id, profile_image_id, clinic_id FROM doctor WHERE id = ? AND ${clause} LIMIT 1 FOR UPDATE`,
      [doctorId, ...cParams]
    );
    ...
    const cid = Number(doctor.clinic_id);
    ...
    const profileImageId = await saveProfileImage(connection, doctor.profile_image_id, payload.profileImage, actorId, cid);
...
async function deleteDoctor(doctorId, scope) {
  const pool = getPool();
  const connection = await pool.getConnection();
  const { clause, params: cParams } = sqlClinicColumn('clinic_id', scope);
  ...
  try {
    await connection.beginTransaction();
    const [doctorRows] = await connection.query(
      `SELECT user_id FROM doctor WHERE id = ? AND ${clause} LIMIT 1 FOR UPDATE`,
      [doctorId, ...cParams]
    );
    if (!doctorRows || !doctorRows[0]) {
      throw new Error('Doctor not found');
