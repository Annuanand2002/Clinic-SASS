...
async function getPatients(req, res, next) {
  try {
    ...
    const q = req.query.q;
    const result = await listPatients(page, limit, q, scopeFromReq(req));
    return res.json({
...
async function createPatientHandler(req, res, next) {
  try {
    ...
    const limit = req.query.limit;
    const q = req.query.q;
    const createdPatientId = await createPatient(payload, actorId, req.clinicId);
    const result = await listPatients(page, limit, q, scopeFromReq(req));
...
async function updatePatientHandler(req, res, next) {
  try {
    ...
    const limit = req.query.limit;
    const q = req.query.q;
    await updatePatient(patientId, payload, actorId, scopeFromReq(req));
    const result = await listPatients(page, limit, q, scopeFromReq(req));
    return res.json({
...
async function deletePatientHandler(req, res, next) {
  try {
    ...
    const limit = req.query.limit;
    const q = req.query.q;
    await deletePatient(patientId, scopeFromReq(req));
    const result = await listPatients(page, limit, q, scopeFromReq(req));
    return res.json({
...
async function getPatientMedicalRecordsHandler(req, res, next) {
  try {
    const patientId = Number(req.params.id);
    const records = await listPatientMedicalRecords(patientId, scopeFromReq(req));
...
async function createPatientMedicalRecordHandler(req, res, next) {
  try {
    ...
    const actorId = req.auth && req.auth.sub ? Number(req.auth.sub) : null;
    const patientId = Number(req.params.id);
    await createPatientMedicalRecord(patientId, req.body || {}, actorId, req.clinicId);
    const records = await listPatientMedicalRecords(patientId, scopeFromReq(req));
    return res.status(201).json({
...
async function deletePatientMedicalRecordHandler(req, res, next) {
  try {
    ...
    await deletePatientMedicalRecord(patientId, recordId, scopeFromReq(req));
    const records = await listPatientMedicalRecords(patientId, scopeFromReq(req));
    return res.json({
