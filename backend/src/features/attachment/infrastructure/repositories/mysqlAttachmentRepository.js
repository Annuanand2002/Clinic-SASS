 ...
 */
async function insertAttachment(payload, actorId, clinicId) {
  const pool = getPool();
  const [result] = await pool.query(
    `INSERT INTO attachment (
      file_name, file_type, base64_data, created_by, patient_id,
      entity_type, entity_id, document_type, title, description, appointment_id, clinic_id
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [
      payload.fileName,
      payload.fileType,
...
async function listAttachmentsByEntity(entityType, entityId, page, limit, includeData, scope) {
  const pool = getPool();
  ...
  const [countRows] = await pool.query(
    `SELECT COUNT(*) AS total FROM attachment
     WHERE entity_type = ? AND entity_id = ? AND ${clause}`,
    [entityType, entityId, ...cParams]
  );
  ...
  const [rows] = await pool.query(
    `SELECT ${cols}
     ...
     ORDER BY id DESC
     LIMIT ? OFFSET ?`,
    [entityType, entityId, ...cParams, limit, offset]
  );
...
async function getAttachmentById(id, scope) {
  const pool = getPool();
  const { clause, params: cParams } = sqlClinicColumn('clinic_id', scope);
  const [rows] = await pool.query(
    `SELECT id, file_name, file_type, base64_data, created_date, patient_id,
            entity_type, entity_id, document_type, title, description, appointment_id
     FROM attachment WHERE id = ? AND ${clause} LIMIT 1`,
    [id, ...cParams]
  );
  return mapRowFull(rows && rows[0]);
}

async function browseAttachments(filters, scope) {
  const pool = getPool();
  ...
  const where = ['entity_type IS NOT NULL', clause];
...
async function deleteAttachmentById(id, scope) {
  const pool = getPool();
  const { clause, params } = sqlClinicColumn('clinic_id', scope);
  const [result] = await pool.query(`DELETE FROM attachment WHERE id = ? AND ${clause}`, [id, ...params]);
 ...
 */
async function updateAttachmentById(id, patch, scope) {
  const cur = await getAttachmentById(id, scope);
  if (!cur) return null;
  ...
  const pool = getPool();
  const { clause, params: wParams } = sqlClinicColumn('clinic_id', scope);
  const [rows] = await pool.query(`SELECT clinic_id FROM attachment WHERE id = ? AND ${clause} LIMIT 1`, [
    id,
    ...wParams
  ]);
  const rowCid = rows?.[0]?.clinic_id != null ? Number(rows[0].clinic_id) : null;
  await pool.query(
  ...
  return getAttachmentById(id, scope);
}
