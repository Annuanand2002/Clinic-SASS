 ...
 */
async function createBill(payload, clinicId) {
  const pool = getPool();
  ...
  try {
    ...
    if (appointmentId) {
      const [ap] = await connection.query(
        `SELECT id FROM appointment WHERE id = ? AND patient_id = ? AND clinic_id = ? LIMIT 1`,
        [appointmentId, patientId, cid]
      );
      if (!ap?.[0]) {
    ...
    const [ins] = await connection.query(
      `INSERT INTO billing (patient_id, appointment_id, total_amount, discount, final_amount, status, bill_date, clinic_id)
       VALUES (?, ?, ?, ?, ?, 'pending', ?, ?)`,
      [patientId, appointmentId, totalAmount, discount, finalAmount, billDate, cid]
    );
...
async function listBills(pageInput, limitInput, scope) {
  const pool = getPool();
  const { page, limit, offset } = normalizePagination(pageInput, limitInput);
  const { clause: countClause, params: countParams } = sqlClinicColumn('clinic_id', scope);
  const { clause: bClause, params: bParams } = sqlClinicColumn('b.clinic_id', scope);

  const [countRows] = await pool.query(`SELECT COUNT(*) AS total FROM billing WHERE ${countClause}`, countParams);
...
async function getBillById(billId, scope) {
  const pool = getPool();
  const { clause: bClause, params: bParams } = sqlClinicColumn('b.clinic_id', scope);
  const [rows] = await pool.query(
    `SELECT
 ...
 */
async function createPayment(payload, scope) {
  const pool = getPool();
  const { clause: bClause, params: bParams } = sqlClinicColumn('clinic_id', scope);
  ...
  try {
    ...
    const [brows] = await connection.query(
      `SELECT id, final_amount, clinic_id FROM billing WHERE id = ? AND ${bClause} FOR UPDATE`,
      [billingId, ...bParams]
    );
    if (!brows?.[0]) {
    ...
    const cid = brows[0].clinic_id != null ? Number(brows[0].clinic_id) : null;
    ...
    const [pins] = await connection.query(
      `INSERT INTO payments (billing_id, amount, payment_method, payment_date, clinic_id) VALUES (?, ?, ?, ?, ?)`,
      [billingId, amount, method, paymentDate, cid]
    );
    const paymentId = pins.insertId;

    await connection.query(
      `INSERT INTO transactions (type, amount, category, reference_type, reference_id, payment_method, transaction_date, clinic_id)
       VALUES ('income', ?, 'patient_payment', 'payment', ?, ?, ?, ?)`,
      [amount, paymentId, method, paymentDate, cid]
    );
...
async function listPayments(pageInput, limitInput, scope) {
  const pool = getPool();
  const { page, limit, offset } = normalizePagination(pageInput, limitInput);
  const { clause: countClause, params: countParams } = sqlClinicColumn('clinic_id', scope);
  const { clause: pClause, params: pParams } = sqlClinicColumn('p.clinic_id', scope);

  const [countRows] = await pool.query(`SELECT COUNT(*) AS total FROM payments WHERE ${countClause}`, countParams);
  const total = Number(countRows?.[0]?.total || 0);
 ...
 */
async function createExpense(payload, scope) {
  const pool = getPool();
  let cid;
  ...
  try {
    ...
    if (linkInventory) {
      ...
      const { clause: iClause, params: iParams } = sqlClinicColumn('clinic_id', scope);
      const [it] = await connection.query(
        `SELECT id, clinic_id FROM inventory_item WHERE id = ? AND is_active = 1 AND ${iClause} LIMIT 1`,
        [itemId, ...iParams]
      );
      if (!it?.[0]) {
      ...
      const { singleClinicId, organizationId } = scope;
      if (singleClinicId != null && Number(singleClinicId) > 0) {
        cid = Number(singleClinicId);
      } else {
        const err = new Error('Select a single clinic to record an expense without inventory purchase');
        err.statusCode = 400;
        throw err;
      }
    }
    ...
    const [eins] = await connection.query(
      `INSERT INTO expense (
        ...
        reference_type, reference_id, description, clinic_id
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        title,
        amount,
    ...
    await connection.query(
      ...
      [amount, category, expenseId, method, expenseDate, cid]
    );
...
async function listExpenses(pageInput, limitInput, scope) {
  const pool = getPool();
  const { page, limit, offset } = normalizePagination(pageInput, limitInput);
  const { clause: countClause, params: countParams } = sqlClinicColumn('clinic_id', scope);
  const { clause: eClause, params: eParams } = sqlClinicColumn('clinic_id', scope);

  const [countRows] = await pool.query(`SELECT COUNT(*) AS total FROM expense WHERE ${countClause}`, countParams);
  const total = Number(countRows?.[0]?.total || 0);
...
async function listLedger(pageInput, limitInput, filters = {}, scope) {
  const pool = getPool();
  const { page, limit, offset } = normalizePagination(pageInput, limitInput);
  ...
  const { clause: tClinic, params: tParams } = sqlClinicColumn('t.clinic_id', scope);
 ...
 */
async function getDashboardSummary(options = {}, scope) {
  const pool = getPool();
  const { clause: txClause, params: txParams } = sqlClinicColumn('clinic_id', scope);
  const { clause: txClause2, params: txParams2 } = sqlClinicColumn('clinic_id', scope);
  const { clause: txClause3, params: txParams3 } = sqlClinicColumn('clinic_id', scope);
  const { clause: txClause4, params: txParams4 } = sqlClinicColumn('clinic_id', scope);
  const { clause: txClause5, params: txParams5 } = sqlClinicColumn('clinic_id', scope);
  const { clause: bPendingClause, params: bPendingParams } = sqlClinicColumn('b.clinic_id', scope);
  ...
  const [inc] = await pool.query(
    `SELECT COALESCE(SUM(amount), 0) AS s FROM transactions
     WHERE type = 'income' AND ${txClause} AND transaction_date >= ? AND transaction_date <= ?`,
    [...txParams, fromDate, toDate]
  );
  const [exp] = await pool.query(
    `SELECT COALESCE(SUM(amount), 0) AS s FROM transactions
     WHERE type = 'expense' AND ${txClause2} AND transaction_date >= ? AND transaction_date <= ?`,
    [...txParams2, fromDate, toDate]
  );
  ...
  const [pendingRows] = await pool.query(
    ...
    WHERE b.status IN ('pending', 'partial') AND ${bPendingClause}
  `,
    bPendingParams
  );
  ...
  const [chartRows] = await pool.query(
    ...
    FROM transactions
    WHERE ${txClause3} AND transaction_date >= ? AND transaction_date <= ?
    GROUP BY DATE_FORMAT(transaction_date, '%Y-%m')
    ORDER BY ym ASC
  `,
    [...txParams3, fromDate, toDate]
  );
